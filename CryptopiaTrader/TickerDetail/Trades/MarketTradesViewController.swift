...
//  CryptopiaTrader
//
//  Created by Florin Uscatu on 10/2/17.
...
class MarketTradesViewController: UITableViewController {
    ...
    func requestDataLocally() {
        if key != "" && secret != "" {
            ApiCalls().requestData(publicType: false, method: "GetTradeHistory", parameters: ["TradePairId":coinId], key: key, secret: secret) { (data) in
                do {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if key != "" && secret != "" {
            ...
            noDataLabel.text = "Please log in to see trades"
            tableView.separatorStyle = .none
            tableView.backgroundView = noDataLabel
        }
        return numberOfRows
